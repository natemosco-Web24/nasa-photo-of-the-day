...
export default function Favorite1(props) {
    const StyledDiv = styled.div`
        ...
        min-height:160vh;
        height-auto;
        background: url(${props.favDay1.hdurl}) no-repeat 100%/cover;
        display:flex;
        justify-content:center;
        align-items:center;
        padding: 0 50px;
    ...
    const renderMedia = () => {
        if (props.favDay1.media_type === "image") {
            return (
                <StyledDiv>
                    <div className="text">
                        <p className="title">{props.favDay1.title}</p>
                        <p className="explanation">{props.favDay1.explanation}</p>
                    </div>
                </StyledDiv>
            )
        } else {
